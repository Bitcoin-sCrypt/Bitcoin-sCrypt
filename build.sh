...
BTCS_DIR=bitcoin-scrypt
BTCSD_EXEC=bitcoin-scryptd
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/bitcoin-scrypt-qt.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    BTCSD_EXEC=bitcoin-scryptd.exe
fi
...
CONFIG="USE_UPNP=- BDB_LIB_PATH=/mingw64/lib BDB_INCLUDE_PATH=/mingw64/include";

echo "running qmake with config $CONFIG";

$QMAKE ../Bitcoin-sCrypt.pro $CONFIG || exit
$MAKE || exit 
